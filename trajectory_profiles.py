...
class Profile:
    def __init__(self, initial_profile_point, final_profile_point, robot_kinematics):
        ...
        self._phase_list = []
        self.reached_point = Tp(0, 0, 0, 0)   # i.e. reached point at the end of the profile.
    ...
    def SampleByPosition(self, positions, time_step=None):
        ...
        for i in range(len(self._phase_list)):
            if IsGreater(self._phase_list[i].end.t, 0.0):
    ...
    def _Set_Start(self, trajectory_point):
        self._start.CopyFrom(trajectory_point)

    def _Get_Start(self):
        return Tp.FromTrajectoryPoint(self._start)

    def _Set_End(self, trajectory_point):
        self._end.CopyFrom(trajectory_point)

    def _Get_End(self):
        return Tp.FromTrajectoryPoint(self._end)
...
class SustainedPulse(Profile):
    ...
    def __init__(self, initial_profile_point, final_profile_point, robot_kinematics):
        ...
        # Phase Linking, this results in automatically updating next phase start when previous phase end changes.
    ...
    def CalculateAtMaxParameters(self):
        ...
        self.UpdateProfilePhases(cruise_end_vel)
    ...
    def ReduceToReachEndPosition(self):
        ...
        cruise_end_vel = self.a_max * cruising_period + self._cruise.start.vel
        self.UpdateProfilePhases(cruise_end_vel)

    def UpdateProfilePhases(self, cruise_end_vel):
        self._cruise.Calculate(cruise_end_vel)
        self._ramp_down.Calculate()
        # Dwell on final velocity to cover any remaining distance if it exists.
...
class Pulse(Profile):
    """An acceleration profile composed of 3 quintic trajectories/phases linked to each other.
        ...
        This is considered a special case of Sustained pulse where Cruise Phase in removed.
    ...
    def CalculateAtMaxParameters(self):
        ...
        # If peak acceleration to reach v_max is higher than a_max, then we cannot go to v_max,
        # Thus use a_max as the peak acceleration.
        ...
        # Dwell on final velocity to cover any remaining distance if it exists.
        self._dwell.Calculate(self._end.pos)
    ...
    def ReduceToReachEndPosition(self):
        ...
        # Dwell on final velocity to cover any remaining distance if it exists (it shouldn't exit).
        self._dwell.Calculate(self._end.pos)
